...
//			}
	 ...
	 * @throws IOException
	 */
	public void cljRun(List<Point> humans, Point p) throws IOException {
		//把人类下的最后一步棋子去除
		allFreePoints.remove(humans.get(humans.size()-1));
				
		RT.loadResourceScript("gobang/core.clj");
		Var foo = RT.var("gobang.core", "do-analysis");
		
		Point result = (Point)foo.invoke(myPoints,humans,allFreePoints);
		System.out.println("ai [" + result.x + "," + result.y + "]");
	...
	public static void main(String[] args) throws IOException {
		...
		List<Point> myChesses = new ArrayList<Point>();
		myChesses.add(new Point(3,3));
		myChesses.add(new Point(2,3));
		
		List<Point> enemyChesses = new ArrayList<Point>();
		enemyChesses.add(new Point(3,4));
		enemyChesses.add(new Point(2,4));
		
		Point result = (Point)foo.invoke(myChesses,enemyChesses,null);
